...
interface CashSubmissionLog {
  id: number;
  order_id: number;
  delivery_boy_name: string;
  cash_submitted: string | number;
...
export default function CashCollectionHistory({ onLogout, onNavigate }: CashCollectionHistoryProps) {
  const [hoveredBtn, setHoveredBtn] = useState<string | null>(null);
  
  // Data States
  const [history, setHistory] = useState<CashSubmissionLog[]>([]);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${BASE_URL}/api/delivery-boys/cash-history`, { params });
      if (res.data.success) {
        setHistory(res.data.data);
      }
    } catch (err) {
      console.error("Error fetching cash submission history:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    const csvHeaders = "ID,Order ID,Delivery Boy,Cash Collected,Cash Submitted,Submission Status,Submitted At";
    const csvRows = history.map(h => 
      `"${h.id}","${h.order_id}","${h.delivery_boy_name}","${parseFloat(String(h.cash_collected)).toFixed(2)}","${parseFloat(String(h.cash_submitted)).toFixed(2)}","${h.submission_status}","${h.submitted_at}"`
    );
    const csvContent = "data:text/csv;charset=utf-8," + [csvHeaders, ...csvRows].join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `cash_submission_history_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const headers = [
    ...
    { label: "SUBMITTED AT", width: "160px" }
  ];
    ...
    <div className="p-8 font-sans selection:bg-blue-500/30 text-white min-h-screen bg-[#070b14]">
      ...
      <div className="dashboard-card p-6 shadow-2xl overflow-hidden bg-[#111827] border-[#1e293b] mt-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-white text-[18px] font-bold tracking-tight">Cash Submission History</h1>
            <nav className="flex items-center gap-2 text-[12px] mt-1">
              ...
              <span className="text-blue-200/80">Submission History</span>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              onClick={() => onNavigate('delivery-boys-cash-collections')}
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-6 py-28 text-center bg-[#0c101a]">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <p className="text-[16px] text-slate-400 font-medium tracking-wide">Loading submission history...</p>
                    </div>
                  </td>
                </tr>
                ...
                paginatedHistory.map((log) => (
                  <tr key={log.id} className="hover:bg-[#141b2d] border-b border-[#1e293b]">
                    ...
                    <td className="px-4 py-3.5 border-b border-[#1e293b] text-slate-300 text-[13px]">{formatCurrency(log.cash_collected)}</td>
                    <td className="px-4 py-3.5 border-b border-[#1e293b] text-emerald-400 font-bold text-[14px]">{formatCurrency(log.cash_submitted)}</td>
                    ...
                    <td className="px-4 py-3.5 border-b border-[#1e293b] text-slate-400 text-[12px]">{formatDate(log.submitted_at)}</td>
                  </tr>
                ))
              )}
            </tbody>
