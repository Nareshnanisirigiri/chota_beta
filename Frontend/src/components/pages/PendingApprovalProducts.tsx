...
export default function PendingApprovalProducts({ onLogout, onNavigate }: PendingApprovalProductsProps) {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        // Filter for non-approved products
        const allProducts = response.data.data || [];
        const pendingOrRejected = allProducts.filter((p: Product) => 
          String(p.approvalStatus).toLowerCase() !== 'approved' && 
          String(p.approvalStatus).toLowerCase() !== 'active'
        );
        setProducts(pendingOrRejected);
      } else {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `pending_products_export_${new Date().toISOString().slice(0, 10)}.csv`);
      ...
      <div className="dashboard-card p-6 shadow-2xl overflow-hidden bg-[#111827] border-[#1e293b] mt-8">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center min-h-[300px] border border-[#2d3748] rounded-md bg-[#0c101a]">
            ...
            <p className="text-slate-400 text-[13px] mt-4 font-light uppercase tracking-widest">Querying Pending Products...</p>
          </div>
        ) : (
                    ...
                    <tr key={row.id} className="hover:bg-slate-800/10 transition-colors border-b border-[#2d3748]/50">
                      ...
                      <td className="border-r border-[#2d3748]/30" style={{ paddingLeft: '16px', paddingRight: '16px', paddingTop: '15px', paddingBottom: '15px' }}>
                        <div className="flex justify-start">
                          <span className={`px-3 py-1 rounded-full uppercase border text-[11px] font-medium tracking-wider bg-amber-500/10 text-amber-400 border-amber-500/20`}>
                            {row.approvalStatus}
                          </span>
                        </div>
                      </td>
